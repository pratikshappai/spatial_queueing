...
def boxplot_arrivals(year, month, max_date=30, dataset="nyc", testing=False):
    if dataset == "nyc":
        dataset_path = f"yellow_tripdata_{year}-{month:02d}.parquet"
        entire_df = pd.read_parquet(dataset_path)
        pickup_datetime = pd.to_datetime(
            entire_df["pickup_datetime"], format="%Y-%m-%d %H:%M:%S"
        )
    elif dataset == "chicago":
        dataset_path = f"Chicago_year_{year}_month_{month:02d}.csv"
        entire_df = pd.read_csv(dataset_path)
        pickup_datetime = pd.to_datetime(
            entire_df["Trip Start Timestamp"], format="%m/%d/%Y %I:%M:%S %p"
        )
    else:
    ...
    for date in range(1, max_date + 1):
        for hour in range(24):
            ...
            n_trips = len(
                pickup_datetime[
                    (pickup_datetime >= min_datetime)
                    & (pickup_datetime < max_datetime)
                ]
            )
    ...
    if testing is True:
        fig.savefig(
            f"MS_R_and_R_Plots/{dataset}_boxplot_trip_demand_year_{year}_month_{month}.png"
        )
    else:
        fig.savefig(
            f"MS_R_and_R_Plots/{dataset}_boxplot_trip_demand_year_{year}_month_{month}.eps",
            bbox_inches="tight",
        )
    plt.close(fig)
...
if __name__ == "__main__":
    boxplot_arrivals(
        year=2022, month=6, max_date=30, dataset="chicago", testing=True
    )
