...
def fleet_size_ode(df_arrival_sequence, start_datetime):
    n_cars = 700
    n_chargers = 300
    list_soc = []
    ...
    total_sim_time_datetime = (
        last_trip_pickup_datetime - first_trip_pickup_datetime
    )
    ...
    charge_time_min = (
        0.2 * SimMetaData.pack_size_kwh / SimMetaData.charge_rate_kw * 60
    )
    total_workload = 0
    served_workload = 0
    for current_index in range(
        1, int(sim_duration_min / SimMetaData.demand_curve_res_min)
    ):
        current_min_total = (
            current_index * SimMetaData.demand_curve_res_min
            + start_day * 1440
            + start_hour * 60
            + start_minute
        )
        ...
        current_min = (
            current_min_total - current_day * 1440 - current_hour * 60
        )
        pickup_time_min = (
            pickup_time_const / (max(n_cars - n_active_trips, 1)) ** 0.5
        )
        drive_to_charger_time_min = (
            drive_to_charger_time_const
            / (max(n_chargers - n_cars_charging, 1)) ** 0.5
        )
        df_pickup_trips = df_arrival_sequence[
            ...
            & (
                df_arrival_sequence["pickup_datetime"]
                + timedelta(minutes=pickup_time_min)
                > datetime(
                    start_year,
                    start_month,
                    current_day,
        ...
        df_en_route_trips = df_arrival_sequence[
            ...
            & (
                df_arrival_sequence["dropoff_datetime"]
                + timedelta(minutes=pickup_time_min)
                > datetime(
                    start_year,
                    start_month,
        ...
        if denom > 0:
            n_cars_driving_without_passenger = (
                n_cars_driving_without_passenger * n_active_trips / denom
            )
            n_cars_driving_with_passenger = (
                n_cars_driving_with_passenger * n_active_trips / denom
            )
        else:
            n_cars_driving_without_passenger = 0
            n_cars_driving_with_passenger = 0
        n_cars_driving_to_charger = (
            min((n_cars - n_active_trips), n_chargers)
            * drive_to_charger_time_min
        ...
        soc_plus = (
            n_cars_charging
            * SimMetaData.charge_rate_kw
            * SimMetaData.demand_curve_res_min
            / 60
            / n_cars
        ...
        soc_minus = (
            (n_active_trips + n_cars_driving_to_charger)
        ...
        list_n_cars_driving_without_passenger.append(
            n_cars_driving_without_passenger
        )
        list_n_cars_driving_with_passenger.append(
            n_cars_driving_with_passenger
        )
...
if __name__ == "__main__":
    ...
    workload_percentage = fleet_size_ode(df_arrival_sequence, start_datetime)
